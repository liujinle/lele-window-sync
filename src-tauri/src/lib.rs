...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            launch_app, kill_process, kill_process_by_name,
        ])
...
#[tauri::command]
fn launch_app(path: String, args: Vec<String>) -> Result<u32, String> {
    use std::process::Command;
    use std::os::windows::process::CommandExt;
    const CREATE_NO_WINDOW: u32 = 0x08000000;

    if path.to_lowercase().ends_with(".lnk") {
        // .lnk: 用 ShellExecute 直接打开，返回目标进程 PID
        // ShellExecute 是 Windows 解析快捷方式的标准方式
        unsafe {
            ...
            // ShellExecute 返回值 > 32 表示成功，但不返回 PID
            // 记录一个伪 PID，后续通过窗口名来关闭
            if result.0 as usize > 32 {
                return Ok(u32::MAX); // 特殊标记，表示用 lnk 启动
            }
            return Err(format!("ShellExecute 失败: {}", result.0 as usize));
        }
    }

    // .exe / 普通程序：直接 spawn，拿到真实 PID
...
fn kill_process(pid: u32) -> Result<(), String> {
    // pid == u32::MAX 表示是 lnk 启动，无法直接用 PID 关闭
    if pid == u32::MAX {
        return Err("LNK 启动的进程无法通过 PID 关闭，请手动关闭".to_string());
    }
    unsafe {
        ...
        let handle = OpenProcess(PROCESS_TERMINATE, false, pid)
            .map_err(|e| format!("无法打开进程 ({}): {e}", e.code().0))?;
...
#[tauri::command]
fn kill_process_by_name(exe_name: String) -> Result<u32, String> {
    // 通过进程名关闭（适用于 lnk 启动的情况）
    unsafe {
        use windows::Win32::System::Diagnostics::ToolHelp::{
        ...
        if Process32FirstW(snap, &mut entry).is_ok() {
            loop {
                ...
                if proc_name == name_lower || proc_name.starts_with(&name_lower.replace(".exe","")) {
